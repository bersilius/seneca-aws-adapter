...
const seneca = require('seneca')({ test: true,  log: { level: 'warn+' } })
...
const optionsSNS = {
    ...
    awsEventListeners: {
        complete: function(resp) {
            seneca.log.info(`Event "complete" handled!`)
        }
    }
}
...
describe(`${pluginName}`, function() {
    ...
    it('Async - SNS - listPlatformApplications', function(done) {
        const eventListeners = {
            'success': function(resp) {
                expect(resp.data).to.have.property('ResponseMetadata')
                expect(resp.data).to.have.property('PlatformApplications')
                done()
            }
        }

        seneca.act(`role: 'aws', service: SNS, cmd: listPlatformApplications`, { eventListeners: eventListeners }, function(err, data) {
            seneca.log.info(err, data)
        })
    })
})
